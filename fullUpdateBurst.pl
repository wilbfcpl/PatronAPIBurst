...
use constant PATRONTYPE_TEEN => "TEEN";
...
my %trace;
my %results;
# Timestamp for the Patron field PATRON_V2. REGDATE, EDITDATE 
my $edittime;
my $todaydate;
...
my ($patronid, $first, $middle, $last,$grade, $address ,$city , $state , $zip, $status,$dob);

#Arrays holding values for simultaneously forked requests
my (@patronid, @first, @middle, @last,@grade, @address ,@city , @state , @zip, @status,@edittime,@dob);
...
sub setupWSDL
{
  ...
  unless ((defined $call1) && (defined $call2)  && ( defined $call4) ) {
    die "[$local_filename" . ":" . __LINE__ . "] SOAP/WSDL Error $wsdl $call1, $call2  $call4 \n" ;
  }
}
...
sub setPatronParams
{
  ...
  $PatronCreateRequest{SearchID} = $patronid[$current_line];
  ...
  $PatronCreateValues{PatronID}=$patronid[$current_line];
  $PatronCreateValues{FirstName} =   $first[$current_line];
  $PatronCreateValues{MiddleName} = $middle[$current_line];
  $PatronCreateValues{LastName}   = $last[$current_line];
  $PatronCreateValues{UserDefinedFields}{cho_UserDefinedField}{UserDefinedField}{Value}=$grade[$current_line];
  $PatronCreateValues{Addresses}{cho_Address}{Address}{Street}=$address[$current_line];
  $PatronCreateValues{Addresses}{cho_Address}{Address}{City}=$city[$current_line];
  $PatronCreateValues{Addresses}{cho_Address}{Address}{PostalCode}=$zip[$current_line];
  $PatronCreateValues{PatronStatusCode} = $status[$current_line];
  $PatronCreateValues{LastEditDate}=$edittime[$current_line];
  $PatronCreateValues{RegistrationDate}=$edittime[$current_line];
...
for (my $current_block=0; $current_block<=$num_chunks;$current_block++ ) {
  ...
  for (my $current_line=0;  $current_line<$burst_len; $current_line++) {
    ...
    #getPatronInformationAPIResponse($MyResponseStatusCode) || die "[$local_filename" . ":" . __LINE__ . "]  getPatronInformation API Call fail \n " ;
